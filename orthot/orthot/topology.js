export { scan_simple, scan_gate, scan_ramp, scan_upladder, scan_downladder }
...
*/
var scan_simple = function(zone, loc, obj, heading, forward, up=direction.code.UP) {
  let fromCTN = loc
  ...
  if (downramp_or_flat) {
    ...
    if (isTraversable) {   
      ...
      if (ramp) {
        ...
        fromUP = toUP
        ;[adjCTN, toCTN, toHEADING, toFORWARD, toUP, isPortaljump, isTraversable] = zone.getLocalTopology(obj, fromCTN, direction.invert[toUP], fromFORWARD, fromUP)
        if (isTraversable) {
          //_isTraversable &= isTraversable
          ramp = toCTN.getObject_bytype("ramp")
          if (ramp) {              
...
var scan_upladder = function(zone, loc, obj, forward, up=direction.code.UP) {
  ...
  if (isTraversable) {  
    ...
    if (isTraversable) {              
      ...
      let ldr = toCTN.getSideobject_bytype(direction.invert[toFORWARD], "ladder")
      if (ldr) {
...
var scan_downladder = function(zone, loc, obj, forward, up=direction.code.UP) {
