export { Manager, keycode, EventReaderPolicy, EventReaderReturnType, EventReader }
...
*/  
var Manager = function(evt_target){
...
var EventReaderPolicy = function(... commands) {
  let pol = {}
  policy_impl(pol, Array.from(commands) )
  return pol
}
// Alter an existing EventReader policy
var modPolicy = function(policy, ... commands) {
  policy_impl(policy, Array.from(commands))
}
// Generate an altered copy of an existing EventReader policy
var subPolicy = function(policy, ... commands) {
...
var policy_impl = function(policy, commands) {

  let toAction = function(str) {
    let action = EventReaderAction[str.toUpperCase()]
    if (action) {
      return action
    }
    console.log("UNRECOGNIZED EventReaderAction:  '" + str + "'")
    return EventReaderAction.IGNORE
  }
...
var EventReader = function(evt_manager, priority=0, policy=null) {
  //Associative array:  "event-name":<read OR consume OR ignore OR drop>
  this.policy = policy ? policy : { default:EventReaderAction.IGNORE }
  this.priority = priority
  this.returnType = EventReaderReturnType.Object      
  ...
  this.receiveEvent = function(code, data) {
    ...
    switch(pol) {
      case EventReaderAction.READ:
        this.RECV(this.pack(code, data))
        return false
      break
      case EventReaderAction.CONSUME:
        this.RECV(this.pack(code, data))
        return true
      break
      case EventReaderAction.IGNORE:
        return false
      break
      case EventReaderAction.DROP:
        return true
      break
      case EventReaderAction.DEFER:
  ...
  this.pack = function(code, data) {      
    switch(this.returnType) {
      case EventReaderReturnType.Object:
        return {code:code, data:data}
      break
      case ReadReturnType.Code:
        return code
      break
      case ReadReturnType.Data:
        return data
      break
    }
  }
